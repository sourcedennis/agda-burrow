...
open import Burrow.Event using (LabelClass)
import Burrow.Execution as Ex
...
open import Burrow.Event
open import Burrow.Execution hiding (WellFormed; Execution)
open import Burrow.Properties
open import Burrow.DerivedWellformed
-- Local imports: Proof Components
import Burrow.Framework.General LabelDst dst dst-wf as Ψ
-- Local imports: Other
open import Burrow.Internal.Helpers as H
...
module Types
  (ev[⇐] : {x : Event LabelDst} → (x∈dst : x ∈ events dst) → Event LabelDst)
  (elim-ev : Event LabelDst)
  where

  CPred[$⇒] : Pred (Event LabelDst) ℓzero → Pred (Event LabelDst) ℓzero → Set
  CPred[$⇒] Pˢ Pᵗ = ∀ {x}
    → x ≢ elim-ev
    → (x∈dst : x ∈ events dst)
    → Pˢ (ev[⇐] x∈dst)
  ...
  CRel[$⇒] : Rel (Event LabelDst) ℓzero → Rel (Event LabelDst) ℓzero → Set
  CRel[$⇒] Rˢ Rᵗ = ∀ {x y}
    → x ≢ elim-ev → y ≢ elim-ev
  ...
  CPred[$⇒]² : Pred (Event LabelDst) ℓzero → Set
  CPred[$⇒]² P = CPred[$⇒] P P
...
record EliminationFramework : Set₁ where
  field
    ...
    src-co   : Rel (Event LabelDst) ℓzero
    src-rf   : Rel (Event LabelDst) ℓzero
...
module Definitions (δ : EliminationFramework) where
  ...
  CRel[⇒]² : Rel (Event LabelDst) ℓzero → Set
  CRel[⇒]² R = CRel[⇒] R R
